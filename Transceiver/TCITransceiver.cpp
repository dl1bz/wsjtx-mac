...
void TCITransceiver::register_transceivers(logger_type *, TransceiverFactory::Transceivers * registry, unsigned id1,
    unsigned id2) {
...
int TCITransceiver::do_start() {
  ...
  stream_audio_ = false;
  _power_ = false;
  CAT_TRACE("TCITransceiver entered TCI do_start and url " + url_.toString() + " rig_power:" + QString::number(
                    rig_power_) + " rig_power_off:" + QString::number(rig_power_off_) + " tci_audio:" + QString::number(
                    tci_audio_) + " do_snr:" + QString::number(do_snr_) + " do_pwr:" + QString::number(do_pwr_) + '\n');
...
void TCITransceiver::onMessageReceived(const QString &str) {
  ...
  for (QString cmds : cmd_list) {
    ...
    if (idCmd != Cmd_Power && idCmd != Cmd_SWR && idCmd != Cmd_Smeter && idCmd != Cmd_AppFocus && idCmd != Cmd_RxSensors &&
        idCmd != Cmd_TxSensors) { printf("%s TCI message received:|%s| ", QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(), str.toStdString().c_str()); printf("idCmd : %d args : %s\n", idCmd, args.join("|").toStdString().c_str());}
    qDebug() << cmds << idCmd;
    if (idCmd <= 0) { continue; }
    switch (idCmd) {
    case Cmd_Smeter:
    ...
    case Cmd_Power:
      ...
      printf("%s Cmd_Power : %s %d\n",
             QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(),
             args.join("|").toStdString().c_str(), power_);
      break;
    case Cmd_VFO:
      printf("%s Cmd_VFO : %s\n", QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(),
             args.join("|").toStdString().c_str());
      printf("band_change:%d busy_other_frequency_:%d timer1_remaining:%d timer2_remaining:%d", band_change,
             busy_other_frequency_, tci_timer7_->remainingTime(), tci_timer2_->remainingTime()); //was timer1 and timer2
    ...
    case Cmd_Mode:
      ...
      if (args.at(0) == rx_) {
        if (ESDR3 || HPSDR) {
          if (args.at(1) == "0") { mode_ = args.at(2).toLower(); }
...
void TCITransceiver::rig_split() {
  printf("%s TCI rig_split:%d->%d busy:%d\n",
         QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(), split_, requested_split_,
         busy_split_);
  ...
  if (tci_Ready && _power_) {
    if ((rx_frequency_ != requested_rx_frequency_)) {
      ...
      if (!band_change2) {
        ...
        printf("rx_frequency is %s and requested_rx_frequency is %s\n", rx_frequency_.toStdString().c_str(),
               requested_rx_frequency_.toStdString().c_str());
        tci_Ready = false;
    ...
    if (!requested_mode_.isEmpty() && requested_mode_ != mode_ && !busy_mode_) {
      ...
      if (requested_mode_.isEmpty() || requested_mode_ == mode_) { update_mode(m); }
        ...
        printf("%s TCI failed set mode %s->%s",
               QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(), mode_.toStdString().c_str(),
               requested_mode_.toStdString().c_str());
      ...
      if (other_frequency_ != requested_other_frequency_) {
        ...
        // if (requested_other_frequency_ == other_frequency_) update_other_frequency (tx);
          ...
          printf("%s TCI failed set txfreq:%s->%s\n",
                 QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(),
                 other_frequency_.toStdString().c_str(), requested_other_frequency_.toStdString().c_str());
...
void TCITransceiver::do_modulator_start(QString mode, unsigned symbolsLength, double framesPerSymbol,
  ...
  printf("%s TCI modulator startdelay_ms=%d ASR=%d mstr=%d mstr2=%d m_ic=%d s_Frames=%lld synchronize=%d m_tuning=%d State=%d\n",
         QDateTime::QDateTime::currentDateTimeUtc().toString("hh:mm:ss.zzz").toStdString().c_str(), delay_ms, audioSampleRate,
         mstr, mstr2, m_ic, m_silentFrames, synchronize, m_tuning, m_state);
  Q_EMIT tci_mod_active(m_state != Idle);
}
